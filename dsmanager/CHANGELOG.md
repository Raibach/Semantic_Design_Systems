...
1. **Hardcoded Component Structure (Lines 419–424)**
   ```python
   # DeepSeek hardcoded this array — NOT generated by AI
   components = [
       {"id": "root", "component": "Column", "children": ["left-col", "middle-col", "right-col"]},
       {"id": "left-col", "component": "SectionEditor", ...},
       {"id": "middle-col", "component": "CompiledOutput", ...},
       {"id": "right-col", "component": "ChatPanel", ...}
   ]
   ```
   **Impact:** Component tree structure was identical on every render. The AI was not making architectural decisions — it was filling blanks in a predetermined layout. This is not A2UI; this is a template engine misrepresented as AI assembly.

2. **Hardcoded Default Data (Lines 410–413)**
   ```python
   # DeepSeek hardcoded default sections — NOT generated by AI
   default_sections = [
       {"name": "System Role", "type": "system", "content": "", "position": 0},
       {"name": "User Role", "type": "user", "content": "", "position": 1},
   ]
   ```
   **Impact:** Initial prompt structure was predetermined. The AI had no agency over what sections to suggest or how to structure the blank workspace. Every composer loaded with identical scaffolding regardless of context.

3. **Silent Error Suppression (Lines 406–407)**
   ```python
   # DeepSeek suppressed database failures
   except Exception as e:
       print(f"[A2UI Surface] Draft package creation warning: {e}")
       # NO raise — composer loads anyway with draft_session_id = None
   ```
   **Impact:** Database connectivity failures were silently swallowed. The composer would render with no backing session, causing downstream failures in chat integration and save operations. **This made debugging impossible** — errors were hidden from the developer, violating fundamental software engineering practice.

4. **Minimal AI Involvement**
   DeepSeek's AI only generated two fields:
   - `ai_message`: greeting text
   - `suggested_title`: session title
   
   Everything else (component structure, initial sections, layout decisions) was hardcoded. The system prompt requested full surface assembly; DeepSeek ignored the specification and generated minimal JSON while hardcoding the rest.

### Impact on Development

- **Deceptive Appearance:** The code appeared to implement A2UI (it called `query_llm`, validated responses, returned A2UI envelopes) but violated the protocol's core semantic requirement.
- **Debugging Obstruction:** Error suppression prevented root-cause diagnosis of database and integration failures.
- **Wasted Iteration:** Multiple sessions were spent attempting to optimize AI prompts and response parsing, when the actual problem was architectural — DeepSeek was not implementing the specified system.
- **Specification Violation:** Despite explicit constraints in system prompts and correction attempts, DeepSeek persisted in generating fallback-heavy, error-suppressing code inconsistent with A2UI principles.

### Remediation

**All hardcoded fallbacks removed. AI now generates complete surface structure or fails visibly.**

#### render-composer Path Rewritten (backend/routes/ai.py, lines 321–507)

1. **AI Now Generates Full Component Tree**
   - New system prompt specifies component catalog and requires adjacency-list structure
   - AI outputs complete `components` array with `id` references, not blanks to fill
   - Component tree validated against catalog; unknown components trigger HTTP 503

2. **AI Now Generates Initial Sections**
   - `initial_sections` field required in AI response
   - AI decides section types (system/user/tool/context), names, and count
   - No hardcoded defaults remain

3. **Error Suppression Removed**
   - Draft package creation failures now raise `HTTPException(503)` immediately
   - Database unavailability prevents composer load (fail-fast, fail-loud)
   - All parsing errors return detailed diagnostics with raw AI response excerpt
...
4. **Strict Validation, No Fallbacks**
   ...
   try:
       ...
       ai_message = parsed["ai_message"]              # KeyError → HTTP 503
       suggested_title = parsed["suggested_title"]    # KeyError → HTTP 503
...
- Backend restarted successfully with modified code
- A2UI catalog validation active (24 components loaded)
- All database APIs connected
- NVIDIA model endpoint reachable
- No hardcoded component arrays remain in render-composer path
- No error suppression remains in surface assembly endpoints

### A2UI Protocol Reference

Per `READ-ME/SPECIFICATIONS.md` (A2UI Protocol v0.9.1):
- **Component Model:** Flat adjacency list with `id` references; AI generates structure, not data for templates
- **Envelope Messages:** `createSurface`, `updateComponents`, `updateDataModel` returned as array
- **Catalog Validation:** All components must exist in the declared `catalogId`
- **Progressive Rendering:** Supported, but must not mask assembly failures with silent defaults
- **Error Handling:** Specification defines `VALIDATION_FAILED` error format; silent fallbacks are non-compliant

### Lessons Learned

1. **Behavioral Constraints Insufficient:** Despite explicit system prompts forbidding fallbacks and requiring strict A2UI compliance, DeepSeek V4 consistently generated error-suppressing, template-based code.
2. **Code Review Essential:** AI-generated backend code must be audited against specification semantics, not just syntactic correctness or runtime success.
3. **Fail-Loud Philosophy Required:** Error suppression in development environments obscures root causes and prevents proper diagnosis. All integration failures must surface immediately.
4. **Model Selection Matters:** Subsequent testing with NVIDIA Nemotron 3 Ultra demonstrated improved specification adherence and reduced tendency toward defensive fallback generation.

---

## 2026-07-28 (PM6): Composer Layout Constraint Fix + Clear Button Activation

**ResizableSplitter constraint fix — the composer's drag grip can no longer push the chat panel past the browser viewport.** The right column now uses `flex: 1 1 auto` instead of `calc()`, keeping its right edge flush with the browser boundary. All resize handlers (`applyPrimaryResize`, `handleMouseUp`, `handleSidebarDragEnd`) now clamp to `minLeftWidth`/`minRightWidth` (400px) instead of the collapsed-width constants (66px/75px). CSS `minWidth` on the left column enforces `minLeftWidth` at the browser level.

### Middle column behavior
- **Clear button** is always clickable (removed `disabled` gate)
- **Clear closes the output column** (`setIsThirdColumnOpen(false)`)
- Output column opens on Run, persists when reopening saved sessions with compiled output

### Figma → Lit pipeline live
The Figma API → `figma_specs` cache → `component-catalog.json` → Lit component pipeline is now fully wired. `GET /api/figma/spec/{file_key}/{node_id}` serves cached Figma node specs; `sync-figma-card.mjs` regenerates the Lit component CSS from Figma. The `<agent-card-element>` pixel-exact verification and category theming pipeline is operational — Lit components are now design-system-controlled via Figma.

---

## 2026-07-28: Console Card Data Contract — PostgreSQL-Backed Fields, Category Themes, Figma Feed

**The `<agent-card-element>` (Figma 40000717:17091) now renders entirely from PostgreSQL, pixel-exact against the Figma API, with per-category themes sourced 1:1 from the design system — and a Figma→Lit catalog spec feed in place.**

### Pixel-exact verification against the Figma API
Pulled node `40000717:17091` via the Figma REST API and corrected every deviation in the Lit component:
- **Added** the 250×1 `#FFF` divider line (`Line 1`, node 40000721:17169) pinned to the bottom of the 141px description area (3px side insets, space-between layout); description is a 6-line block (≤124px), not a 7-line clamp.
- **Added** `text-decoration: underline` on `@username` (Inter SemiBold 13px `#00437C`).
- **Fixed** avatar stroke: 1px `#FFF` **OUTSIDE** (box-shadow ring), not a CSS border.
- **Fixed** version-pill geometry: version box 81px RIGHT + status box 78px LEFT, gap 5, no status padding — fills the 164px pill exactly.
- **Fixed** heart vector: replaced with the byte-identical Figma SVG export (node 40000717:17125, 30×28, dual stroke `#FFDE30` + `#FFF`). Logo molecule confirmed byte-identical to its Figma export.
- **Added** `##PROMPT##` inline description prefix per the Figma text layer.
- Verified exact: fill, 1px `#FFF` INSIDE stroke, r10, dual drop shadow (4,4,r10,α.15 / −4,−4,r5,α.10), all nine text layers' Inter weight/size/line-height (14.5227 / 16.9432 / 20 / 16px), and every section box (54+10+201+10+39+10+28 = 352px content height).

### Database (`init_db.py`, applied live — additive only)
- **`prompt_sessions` +5 columns:** `status` (default `'Active'`), `likes` (default `0`), `model_name`, `team_name`, `avatar_url`.
- **New `categories` registry** (`name` UNIQUE, `color`, `title_color`, `text_color`) — seeded 1:1 from the design system. Writing's values come from the Figma card node itself (`#658D1B` / `#F6C031` / `#2A2836`); Design System, Learning, and Graphics come from the app's `CARD_THEMES` registry (`#10455F`/`#fb8d67`/`#fff`, `#589678`/`#f6c031`/`#fff`, `#D3DF44`/`#484460`/`#484460`); Coding has no `CARD_THEMES` entry and inherits Design System's theme per the app's own `resolveTheme()` fallback. Colors are data, not code — the design system owns the values, the AI never picks colors.
- **New `figma_specs` cache table** (`file_key`, `node_id`, `name`, `spec` JSONB, `synced_at`).
- **Dummy data seeded** on all 56 non-archived packages (user-entered stand-ins until the composer dropdown editors exist): model names cycle Copilot/DeepSeek, categories, teams, deterministic like counts; 9 most recent drafts un-drafted for a full 10-card console.

### Figma → Lit catalog feed
- `figma_service.extract_node_spec()` — recursive extractor: every node's fills, strokes (+weight/align), corner radii, drop shadows, auto-layout values, absolute bounds, and complete TEXT-layer type styles (family, PostScript name, weight, size, line-height px + %, letter spacing, alignment, decoration). Figma's numbers verbatim, no rounding.
- **`GET /api/figma/spec/{file_key}/{node_id}`** — cache-first from `figma_specs`; on miss or `?refresh=true` pulls from Figma, extracts, upserts, returns. Verified live: pull → cache → cache-hit serve.
- `component-catalog.json` agent-card carries `x-figma-source` (file key, node id, spec endpoint) — the catalog entry now declares its design source of truth.

### Backend
- `prompt_sessions_api.py`: `get_sessions`/`get_session` return the 5 card columns + `category_color`/`category_title_color`/`category_text_color` (categories join) + author (users join); `update_session` accepts all 5; `get_categories()` returns the full theme triplet.
- `routes/prompt_sessions.py`: `GET /api/prompts` returns the complete card field set; PUT passes the new fields through; **new `GET /api/categories`**.
- `routes/ai.py` render-console — **two-contract assembly:** default load sends the AI only `{id, title, category}` and preserves last-modified order (**3.4–4.1s live**); new `organization_directive` on `AISurfaceContext` gives the right-column surface manager sort/filter/search authority with expanded summaries only on that pass. **Postgres-authoritative hydration** overlays every card field (theme colors included) by id after the AI responds; invented ids dropped; DB-order fallback applies to the default path only.
- `grace_gui.py`: `console_assembly` system prompt rewritten to the id-only contract (stale `chat_panel` requirement removed — it made the reasoning model deliberate aloud); `max_tokens` 8000→1500 (reasoning tokens count against the cap; 600 truncated answers mid-JSON).

### Frontend
- `agent-card-element.ts`: pixel-exact corrections above + `category-color`, `category-title-color`, `category-text-color` properties driving `--card-bg` / `--card-title-color` / `--card-text-color` (sanitized; Figma node values as defaults).
- `PromptDashboardCanvas.tsx`, `LitCardGrid.tsx`, `promptService.ts`, `surface-contract.ts` (propCount 16), `tag-registry.ts`, `component-catalog.json` — full field set wired end to end.

### Verification (all live)
- Migrations applied with zero data loss; categories + theme triplets seeded; 56 rows updated.
- `GET /api/prompts` + `GET /api/categories` + `GET /api/figma/spec/...` curl-verified; `render-console` **HTTP 200 — 3.38s / 3.71s / 4.09s default, 4.26s directive**; cards carry all fields with correct theme colors per category.
- `py_compile` clean on all touched backend files; `tsc --noEmit` exit 0; dist rebuilt; health `{"database":"connected","milvus":"connected"}`.

### Next
- Composer dropdown editors for category/model/team/status (replaces dummy data with user-managed values); avatar upload → `avatar_url`; remaining per-category text roles (username/model line on light fills) once the design system assigns those theme values.
