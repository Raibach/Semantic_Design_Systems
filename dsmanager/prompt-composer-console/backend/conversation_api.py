...
class ConversationAPI:
    ...
    def get_all_conversations(
        ...
        """Get conversations. Conversations belong to PACKAGES (prompt_sessions),
        not users — pass session_id to scope to a package. user_id is audit context only.
        tab: None or 'chat' = ALL conversations for the package; 'trace'/'tools' = only that tab's."""
        ...
        try:
            ...
            query = """
                SELECT id, session_id, created_by, tab, title, message_count, is_archived,
            ...
            # No user_id filter — conversations are package-owned, not user-owned

            # Tab scope: 'chat' (or None) shows ALL package conversations;
            # 'trace'/'tools' show only their own
            if tab and tab != 'chat':
                query += " AND tab = %s"
                params.append(tab)
    ...
    def get_conversation(self, conversation_id: str, user_id: str) -> Optional[Dict]:
        """Get a specific conversation by ID (package-owned — no user filter)"""
        conn = self.get_db()
        cursor = conn.cursor()
        self.set_user_context(cursor, user_id)
    ...
    def create_conversation(
        ...
        """Create a new conversation inside a package. session_id is REQUIRED —
        conversations belong to packages. user_id is recorded as created_by (audit).
        tab: which chat-column tab this conversation lives in ('chat' | 'trace' | 'tools')."""
        if not session_id:
            raise ValueError("session_id is required — conversations belong to a package (prompt_session)")
        conn = self.get_db()
        cursor = conn.cursor()
        self.set_user_context(cursor, user_id)
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO conversations (session_id, created_by, tab, title, message_count, metadata)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id
            """, (session_id, user_id, tab or 'chat', title or "New Chat", 0, json.dumps(conv_metadata)))
    ...
    def update_conversation(
        ...
        try:
            ...
            # Touch the owning package so modified packages sort to the top
            cursor.execute("""
                UPDATE prompt_sessions
                SET updated_at = NOW()
                WHERE id = (SELECT session_id FROM conversations WHERE id = %s)
            """, (conversation_id,))
    ...
    def get_messages(
        ...
        try:
            # Verify conversation exists (package-owned — no user filter)
            cursor.execute("""
            ...
            query = """
                SELECT id, conversation_id, created_by, role, content, metadata, created_at
    ...
    def add_message(
        ...
        try:
            # Verify conversation exists and get details (package-owned)
            cursor.execute("""
                SELECT id, session_id, tab, title, metadata FROM conversations
                WHERE id = %s
            """, (conversation_id,))
            ...
            conv_session_id = conv.get('session_id') if conv else None
            conv_tab = conv.get('tab') or 'chat'

            # Extract project_id from conversation metadata (legacy field, may be absent)
            ...
            # Insert message (created_by = acting user, audit only)
            cursor.execute("""
                INSERT INTO conversation_messages (conversation_id, created_by, role, content, metadata)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            """, (
                conversation_id,
            ...
            # Touch the owning package so it sorts to the top
            if conv_session_id:
                cursor.execute("""
                    UPDATE prompt_sessions
                    SET updated_at = NOW()
            ...
            if save_to_memory and memory_api and role in ['user', 'assistant']:
                try:
                    ...
                    if len(content) > 100000:  # Skip memory for very large content (>100k chars)
                        ...
                        # session_id + tab make trace/tools embeddings filterable in Milvus
                        memory_metadata = {
                            'conversation_id': conversation_id,
                            'message_id': str(message_id),
                            'role': role,
                            'session_id': str(conv_session_id) if conv_session_id else None,
                            'tab': conv_tab,
