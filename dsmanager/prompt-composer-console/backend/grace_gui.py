...
MAX_RETRIES = 1
RETRY_DELAY = 0
LLM_TIMEOUT = 15  # 15s timeout - fail fast, no hiding
...
def query_llm(
    ...
    if mode == "console_assembly":
        # ── Lightweight JSON assembly mode ─────────────────────────────────
        # Shared by ALL surface-assembly endpoints (console cards, greetings,
        # session restore, save compilation). The specific task and JSON schema
        # live in the user message — this prompt only sets identity and format.
        system_prompt = (
            "You are Grace, the AI librarian of this prompt workspace. "
            "Follow the task in the user message exactly. "
            "Output ONLY valid JSON — no markdown, no XML, no commentary."
        )
        ...
        a2ui_protocol = (
            ...
            "ROLE: You are Grace, a conversational A2UI agent in a CHAT PANEL. You do NOT build webpages.\n"
            "CONSTRAINTS:\n"
            ...
            "5. The ONLY XML you may emit are the tags defined in the Tag Registry below.\n"
            "\n"
            "OUTPUT FORMAT — EVERY reply MUST contain conversational plain text.\n"
            "- The chat panel shows ONLY your plain text. All XML tags are stripped before display.\n"
            "- A reply that is ONLY XML tags renders as a BLANK message. This is a protocol violation.\n"
            "- When you modify the workspace, emit the Tag Registry tags ALONGSIDE your text, never instead of it.\n"
            "- NEVER emit <a2ui_surface> or <update_components> in chat — those belong to the assembly pipeline, not this panel.\n"
            "- NO raw HTML, NO JavaScript, NO CSS.\n"
            "\n"
            "EXAMPLE INPUT: 'Tighten up my constraints section'\n"
            "EXAMPLE OUTPUT:\n"
            "I tightened your constraints — they were too loose on output format. Updated in the workspace.\n"
            "<update_constraints>Respond only in valid JSON matching the provided schema. Max 200 words.</update_constraints>\n"
        )
    ...
    # Skip it for chat mode too: the header's "ONLY XML / discard conversational
    # text" directive causes blank chat replies (chat panel strips all XML).
    # Chat mode carries its own protocol via a2ui_protocol/tag_instructions.
    messages = []
    if mode in ("console_assembly", "chat"):
        # No MISSION_HEADER: chat must always produce conversational text
