...
async def store_dictation_memory(
    ...
    try:
        ...
        try:
            ...
            if state.tag_extractor is None:
            ...
            historical_tags = state.tag_extractor.extract_historical_context_tags(
                request.content
            )
        ...
        if request.memory_id:
            ...
            memory_id = state.memory_api.update_memory(
            ...
            # Create new memory
            memory_id = state.memory_api.create_memory(
                user_id=user_id,
                content=request.content,
                content_type="text",
                source_type="dictation",
...
async def delete_memory(
    ...
    try:
        ...
        cursor = None
        try:
            # Use RealDictCursor to match state.memory_api pattern (it uses dict access like cursor.fetchone()['id'])
